...
int start4(char *arg)
{
   ...
   console("start4(): disk scheduling test, create 4 processes that write\n");
   console("          various places on the disk.  After the 4 processes are\n");
   console("          done, start4 reads the 4 locations & prints the\n");
   console("          results.  Uses disk 1.\n");
	...
	if (DiskRead(XXbuf[j], 1, j, j, 1, &status) < 0) {
	    console("start4(): ERROR: DiskGet\n");
	}
	if (status < 0)
	  console("start4(): disk_get returned error");
	console("start4(): read: %s", XXbuf[j]);
    }

    console("start4(): done %d\n",cksum);
    Terminate(2);
    return 0;
} /* start4 */
